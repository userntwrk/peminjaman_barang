<?php
           ...
           // echo $file_name." ".$file_type." ".$file_size." ".$nama_barang." ".$stok_barang." ".$extension;
            if(($extension == "jpg" || $extension == "jpeg" || $extension == "gif" || $extension == "png") && ($file_type == "image/jpeg" || $file_type == "image/png" || $file_type=="image/gif") && $extension == $file_size<=$max_size){
                ...
                if (move_uploaded_file($tmp_name, $location.$file_name)) {
                    if(mysqli_query($koneksi, "UPDATE tbl_barang SET nama_barang='$nama_barang', merk_barang='$merk_barang', gambar_barang='$file_name', stok_barang='$stok_barang', tahun_barang='$tahun_barang' WHERE id='$id'")){
	...
	if(isset($_GET['id'])){
		$id = $_GET['id'];
		$query = mysqli_query($koneksi, "SELECT * FROM tbl_barang WHERE id='$id'");
		$data  = mysqli_fetch_array($query);
		$nama_barang   = $data['nama_barang'];
        $merk_barang   = $data['merk_barang'];
		$gambar_barang = $data['gambar_barang'];
		$stok_barang   = $data['stok_barang'];
        $tahun_barang   = $data['tahun_barang'];
	}
?>
                    ...
                    <div class="col-lg-6">
                        <div class="card">
                          ...
                          <form class="card-body card-block" action="" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label for="merk" class=" form-control-label">Merk Barang</label>
                                <input type="text" id="merk" name="merk_barang" class="form-control" value="<?php echo $merk_barang;?>"> 
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label for="tahun" class=" form-control-label">Tahun Barang</label>
                                <input type="number" id="tahun" name="tahun_barang" value="<?php echo $tahun_barang;?>" class="form-control"> 
                            </div>
